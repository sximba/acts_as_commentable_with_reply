...
class ActsAsCommentableTest < Test::Unit::TestCase
  ...
  def test_create_reply
    post = Post.create(:text => "Awesome post !")
    comment = post.comments.create(:title => "comment.", :comment => "This is the a comment.")
    reply = comment.comment_replies.create(:reply => "This is a reply")
    assert_not_nil reply.id
  end
  ...
  def test_fetch_replies
    ...
    comment.comment_replies.create(:reply => "First reply")
    commentable = Post.find(1)
    assert_equal 1, commentable.comments.first.comment_replies.length
    assert_equal "First reply", commentable.comments.first.comment_replies.first.reply
  end
  ...
  def test_find_comment_replies_by_user
    user = User.create(:name => "Mike")
    user2 = User.create(:name => "Fake")
    post = Post.create(:text => "Awesome post !")
    comment = post.comments.create(:title => "First comment.", :comment => "This is the first comment.", :user => user)
    reply = comment.comment_replies.create(:reply => "First reply", :user => user2)
    assert_equal [], CommentReply.find_comment_replies_by_user(comment, user)
    assert_equal [reply], CommentReply.find_comment_replies_by_user(comment, user2)
  end

  def test_find_comment_replies
    user = User.create(:name => "Mike")
    user2 = User.create(:name => "Fake")
    post = Post.create(:text => "Awesome post !")
    comment = post.comments.create(:title => "First comment.", :comment => "This is the first comment.", :user => user)
    reply = comment.comment_replies.create(:reply => "First reply", :user => user2)
    reply2 = comment.comment_replies.create(:reply => "Second reply", :user => user)
    assert_equal [reply, reply2], CommentReply.find_comment_replies(comment)
  end
